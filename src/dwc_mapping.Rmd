...
```{r}
species_profile %<>% mutate(taxonID = taxonID)
```
...
```{r}
species_profile %>%
  select(habitat, isMarine, isFreshwater, isTerrestrial) %>%
  group_by_all() %>%
  summarize(records = n())
```
...
```{r}
species_profile %<>% select(-starts_with(""))
```
...
```{r}
description_ext %<>% mutate(taxonID = taxonID)
```
...
```{r}
description_ext %<>% select(-starts_with(""))
```
