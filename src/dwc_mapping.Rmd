...
```{r}
# Raw files:
raw_data_file = "../data/raw/Checklist5.xlsx"
...
```{r}
taxon %<>% mutate(scientificNameID = raw_scientific_name_id)
```
...
Habitat information can be found in `raw_habitat`, which describes whether a species is found in freshwater, terrestrial or both (terrestrial/freshwater) habitats.

Show unique values:

```{r}
species_profile %>%
  distinct(raw_habitat) %>%
  arrange(raw_habitat)
```
...
```{r}
species_profile %<>% mutate(raw_habitat = str_to_lower(raw_habitat)) 
```
...
species_profile %<>% mutate(isFreshwater = case_when(
  raw_habitat == "fresh water" | raw_habitat == "terrestrial/fresh water" ~ "TRUE",
  TRUE ~"FALSE"
))
```
...
species_profile %<>% mutate(isTerrestrial = case_when(
  raw_habitat == "terrestrial" | raw_habitat == "terrestrial/fresh water" ~ "TRUE",
  TRUE ~"FALSE"
))
```
...
```{r}
species_profile %>%
  select(raw_habitat, isMarine, isFreshwater, isTerrestrial) %>%
  group_by_all() %>%
  summarize(records = n())
```
